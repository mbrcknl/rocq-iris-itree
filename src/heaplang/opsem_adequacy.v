...
Lemma base_Beta f_ x_ e v :
  ...
  ≈ let e' := (subst' x_ v (subst' f_ (RecV f_ x_ e) e))
     in step ;; yield_if_not_val e' ;; compile_expr e'.
Proof.
...
Lemma trace_invariant_ret tp σ n v :
  tp_termination tp σ (TermRet v) →
  trace_invariant σ (TermRet v) n (CTRet v).
Proof.
...
Lemma stuck_ub tp σ :
  tp_termination tp σ TermUb →
  ∃ tid tr, trace_invariant σ TermUb 0 tr ∧ is_ctrace tr tid (compile_tp tp).
Proof.
  intros Hterm.
  assert (Hterm' := Hterm).
...
Lemma step_in_thread e1 σ1 κs e2 σ2 efs tr tid tid' tp (k : val → itree heaplangE val) tx n :
  ...
  is_ctrace tr tid' (<[tid:=(v ← compile_expr_yield e2 ; k v)%itree]>tp
                    ++ compile_tp' efs) →
  trace_invariant σ2 tx n tr →
  tp !! tid = Some (v ← compile_expr_yield e1 ; k v)%itree →
  ∃ tr', trace_invariant σ1 tx (S n) tr' ∧ is_ctrace tr' tid tp.
Proof.
...
Proof.
  ...
  - apply IH in Hstep' as [tid [tr [Hinv Htr]]]; eauto; last first.
    ...
    * rewrite compile_tp_app // in Htr.
      ...
      apply step_in_thread with (tp := (compile_tp tpa ++ (v ← compile_expr e1; yield_if_not_val e1;; compile_expr_kill (fill K (Val v)))%itree :: compile_tp' tpb)) (tid := length tpa) (tid' := tid) (k := λ v, (compile_expr (fill K (Val v));; yield ;; kill_thread)%itree) (tr := tr) (tx := tx) (n := n) in Hbase as [tr' [Hinv' Htr']].
